-- This code is taken from https://github.com/catppuccin/nvim/blob/main/lua/catppuccin/utils/colors.lua (MIT Licence)

local M = {}

local hsluv = require("colorly.utils.hsluv")
...
function M.increase_saturation(hex, percentage)
  ...
  if rgb_intensity.mid == rgb_intensity.min then
    ...
    local intensity_proportion = (rgb_intensity.max - rgb_intensity.mid)
      / (rgb_intensity.mid - rgb_intensity.min)
    new_intensities.mid = (
      intensity_proportion * new_intensities.min + rgb_intensity.max
  ...
  return (
    rgb2Hex { new_intensities.max, new_intensities.min, new_intensities.mid }
  )
end
